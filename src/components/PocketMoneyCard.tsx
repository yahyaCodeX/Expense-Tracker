          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {hasPocketMoney && (
                ...
                >
                  {isOverBudget ? 'Budget Exceeded' : `${spentPercentage}% Spent`}
                </span>
              )}
            </div>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Total allowance & spending limit for {monthName}
            </p>
          </div>
        </div>
        ...
        <div>
          {!isEditing ? (
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 text-xs font-bold text-slate-700 dark:text-slate-200 transition-colors cursor-pointer"
            >
              <Pencil className="w-3.5 h-3.5" />
              <span>{hasPocketMoney ? 'Edit Pocket Money' : '+ Set Pocket Money'}</span>
            </button>
          ) : (
            <button
              type="button"
        ...
        <div className="p-5 sm:p-6 bg-amber-50/50 dark:bg-amber-950/20 border-b border-amber-200/60 dark:border-amber-900/40 space-y-4">
          <div>
            <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">
              Enter Pocket Money Amount (Rs.) for {monthName}
            </label>
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  disabled={isSaving}
                  onClick={() => handleSave()}
                  className="flex-1 sm:flex-none px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-bold text-xs shadow-xs transition-colors flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50 min-h-[38px]"
                >
                  <Check className="w-3.5 h-3.5" />
                  <span>{isSaving ? 'Saving...' : 'Save Amount'}</span>
                </button>
                {hasPocketMoney && (
                  <button
                    type="button"
                    onClick={() => handleSave(0)}
                    title="Remove pocket money setting"
                    className="px-3 py-2 rounded-xl border border-slate-300 dark:border-slate-700 text-slate-500 hover:text-rose-600 dark:hover:text-rose-400 text-xs font-semibold hover:bg-rose-50 dark:hover:bg-rose-950/30 transition-colors"
                  >
                    Clear
                  </button>
                )}
                <button
                  type="button"
                  onClick={handleCloseEdit}
                  className="px-3 py-2 rounded-xl text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 text-xs font-semibold"
                >
                  Cancel
                </button>
              </div>
        ...
        <div className="p-5 sm:p-6 text-center space-y-3">
          <div className="max-w-md mx-auto">
            <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">
              No pocket money added yet for {monthName}
            </p>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              Add your monthly allowance to calculate your remaining balance and recommended daily spending limit.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/70 dark:border-slate-800">
              ...
              <span className="text-xs text-slate-500 dark:text-slate-400 mt-1 block truncate">
                Mess: {formatCurrency(messTotalSpent)} • Other: {formatCurrency(otherExpensesTotalSpent)}
              </span>
            </div>
            ...
            <div
              className={`p-4 rounded-2xl border ${
                isOverBudget
                  ...
                  : remainingBalance <= pocketMoney * 0.15
            ...
            >
              ...
              >
                {isOverBudget
                  ...
                  : `${100 - Math.min(100, spentPercentage)}% of pocket money left`}
              </span>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs font-bold text-slate-600 dark:text-slate-300">
              <span className="flex items-center gap-1.5">
                ...
                <span className="text-slate-400 font-medium">({spentPercentage}%)</span>
              </span>
              <span>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-slate-500 dark:text-slate-400 pt-1">
              ...
              {!isOverBudget && remainingDays > 0 && remainingBalance > 0 && (
              ...
              {isOverBudget && (
                <div className="flex items-center gap-1.5 font-bold text-rose-600 dark:text-rose-400">
                  ...
                  <span>Limit reached. Try minimizing daily expenses.</span>
                </div>
              )}
            </div>
          </div>
