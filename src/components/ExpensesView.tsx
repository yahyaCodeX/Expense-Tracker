...
export const ExpensesView: React.FC<ExpensesViewProps> = ({
  ...
  const [itemToDelete, setItemToDelete] = useState<DailyExpenseItem | null>(null);
  ...
  const handleConfirmDelete = async () => {
    ...
    const titleToDelete = itemToDelete.title;
    const idToDelete = itemToDelete.id;
    setIsDeleting(true);
    try {
      await onDeleteExpense(idToDelete);
      setItemToDelete(null);
      setSuccessToast(`Deleted "${titleToDelete}" successfully.`);
      setTimeout(() => {
        setSuccessToast(null);
      }, 3500);
    } catch (err: any) {
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        id={`delete-daily-expense-${item.id}`}
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-sm w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-800">
            <div className="flex items-center gap-3 text-rose-600 dark:text-rose-400 mb-3">
              ...
              <div>
                ...
                <span className="text-[11px] text-slate-400 font-medium">This cannot be undone</span>
              </div>
            </div>
            <p className="text-xs text-slate-600 dark:text-slate-400 mb-4">
              Are you sure you want to delete <strong className="text-slate-900 dark:text-white">"{itemToDelete.title}"</strong> ({formatCurrency(itemToDelete.amount)})?
            </p>
            <div className="flex items-center justify-end gap-2.5">
              <button
                ...
                disabled={isDeleting}
                className="px-4 py-2 rounded-xl text-xs font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                ...
                disabled={isDeleting}
                className="px-4 py-2 rounded-xl text-xs font-bold bg-rose-600 hover:bg-rose-700 active:scale-95 text-white shadow-sm transition-all cursor-pointer disabled:opacity-50"
              >
